...
glimpse(and_vertebrates)       #na values in dataset
...
(The p-value helps us determine whether the observed result is statistically significant. In the context of the F-test, here's how you interpret it:

Small p-value (< 0.05): This indicates that the observed data is unlikely to occur if the null hypothesis were true (i.e., the variances are equal). In this case, you would reject the null hypothesis and conclude that the variances are significantly different.

Large p-value (≥ 0.05): This indicates that the observed data is consistent with the null hypothesis, and there is insufficient evidence to conclude that the variances are different. Thus, you would fail to reject the null hypothesis and conclude that there is no significant difference in the variances.)
...
cont_saltable <- table(sal_clean$section, sal_clean$unittype)

```

```{r}
#chisq.test() assumes the null hypothesis is that all frequencies have equal probability.

#we want to know if abundance is affected by both channel and and forest type (section). Here, our null hypothesis is that forest and channel type are independent

chisq.test(cont_saltable)
```
The p-value(<2.2e-16) is very small in this test, so we can reject the null hypothesis that forest type and channel type have equal numbers of salamanders. This shows that there is a significant relationship between foresttype and channel type.
...
```{r}
#A t-test assumes the variance of each group is equal and the data are normally distributed.

# we need two vectors of the weights in each separate forest section.
cc_salweight <- sal_clean |> 
  filter(section == "CC") |> 
#You can use pull() to convert a single column of a data frame/tibble to a vector
  pull(weight_g)

og_salweight <- sal_clean |> 
  filter(section == "OG") |> 
  pull(weight_g)

var.test(cc_salweight, og_salweight)
#Small p-value (< 0.05): This indicates that the observed data is unlikely to occur if the null hypothesis were true (i.e., the variances are equal).
```
The null hypothesis that the variance of each group is equal and that this is normally distributed, is rejected. The small p-value (1.439e-07 ) suggests that the relationship between cc_salweight and OG_salweight are significantly different from each other.
<br>
...
```{r}
#which lenth variable represents snout to fork length
?and_vertebrates    
#= length_1_mm
...
# statistically test if a variable fits a normal distribution with the shapiro.test(). can only work with 5000 or less samples
...
```
Both of the cutthroat trout weight_g and length_1_mm variables have a small p-value of less than .05 (<2.2e-16 for both).This suggests that the data won't fit a normal distribution. The next step is to transform the variables or use a non-parametric test.

```{r}
#view if normal distribution with log transformation first
ggarrange(
 gghistogram(log(cutthroat_clean$length_1_mm), title = "Length"), 
 gghistogram(log(cutthroat_clean$weight_g), title = "Weight") )
```
The log transformation doesn't present a normal distribution on the histogram plot, but instead a bimodal one. It's best to move forward with a a spearman correlation test, which works with non-parametric/ non-linear data.

```{r}
cor.test(cutthroat_clean$length_1_mm, cutthroat_clean$weight_g, method = "spearman")
```
These results show that there is a strong positive correlation between cutthroat trout length and weight. The pvalue (<2.2e-16) is less than .05 which signals a significant relationship between the two variables. The correlation efficient (.99) is also high and verifies the relationship. As the length of the cutthroat trout increases, so does the weight.
...
ANOVA assumes normal distributions within each group. We can utilize out nest/map approach to assess the normality of our data. We will use the Shapiro-Wilk normality test, which is a good test for small sample sizes.

```{r}
norms <- pie_crab |> 
  nest(data = -site) |> #the data is being grouped by all columns except for site. The result is a new column data that contains a nested data frame for each site.
  mutate(Shapiro = map(data, ~ shapiro.test(.x$size)), #This applies the Shapiro-Wilk test (shapiro.test) for normality to the size variable within each nested dataset. map() used to apply a function to each element of a list. Here, it applies the shapiro.test() function to each data column (which contains nested datasets) and performs the test on the size variable.
         n = map_dbl(data, nrow),#This calculates the number of rows (sample size) in each nested dataset (each group of site) and stores it in the n column. map_dbl() is a variation of map() that ensures the result is returned as a numeric vector.
         glance_shapiro = map(Shapiro, broom::glance)) |>
  unnest(glance_shapiro) #The glance() function extracts useful summary statistics from the test result object, such as the test statistic, p-value, and other information, and stores it in a tidy format.
...
broom::augment(pie_lm, newdata = new_lat) #adding in info
```
...
```{r}
# Filter and order sites by latitude

#reorder the site factor by using arrange() to ensure that the sites are ordered by latitude and avoid duplicate levels. 
pie_sites1 <- pie_crab %>%
  filter(site %in% c("GTM", "DB", "PIE")) %>%
  arrange(latitude) %>%
...
# Create the boxplot
ggplot(pie_sites1, aes(x = site, y = size, fill = site)) +
  geom_boxplot() +
...
Within the pie_crab data set, we looked at Fiddler crab body size to check for significant differences in carapace width (size) across three different sites of GTM, DB, and PIE. Within each site there is a different latitude that corresponds with it.To test for significant differences between the size and site an ANOVA test was performed. This test revealed that at least one of the group means was significantly different from the others with a p-value of 2e-16. To check which pairs of sites were significantly different from each other the Tukey's HSD post-hoc was used. The results of that test showed that the greatest difference between carapace width (size) between sites was greatest between GTM and PIE with a diff value of 6.100714.
...
pie_lm_watert<- lm(size ~ water_temp_sd, data = pie_crab)

# View the results of the linear model
summary(pie_lm_watert)
...
In a linear model prediction between the water temperature and crab size, the pvalue (.528) shows that the water temperature doesn't have a significant impact on crab size. The linear regression model also shows the same result.
...
#with multiple regression, one of the assumptions is that there is no correlation between the predictor variables.
pie_lm2 <- lm(size ~ latitude + air_temp_sd + water_temp_sd, data = pie_crab)

summary(pie_lm2)

#test for correlations among more than two variables with the cor() function
pie_crab %>%
  dplyr::select(latitude, air_temp_sd, water_temp_sd) %>%
  cor()
```
The multiple linear regression model between the three predictor variables suggests that there is a significant impact of the combination of the predictors on the crab size with an overall p-value <2.2e-16. However, water_temp_sd appears to not have a significant impact.
