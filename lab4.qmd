...
```{r}
#use the distinct() function to ensure that we only consider one row per site for ordering.

#Reorder 'site' based on 'latitude' and ensure unique levels for 'site'
pie_crab1 <- pie_crab %>%
  distinct(site, .keep_all = TRUE) %>%   # Ensure only one row per site
  mutate(site = factor(site, levels = pie_crab$site[order(pie_crab$latitude)]))

# Create the boxplot
ggplot(pie_crab1, aes(x = site, y = size, fill = site)) +
  geom_boxplot() +
